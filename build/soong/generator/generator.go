...
func init() {
	android.RegisterModuleType("hertzify_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = hertzifyExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = hertzifyExpandVariables(ctx, dep_file)
	...
	cmd := hertzifyExpandVariables(ctx, String(g.properties.Cmd))
