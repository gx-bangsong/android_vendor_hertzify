...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from HertzifyOS-Devices Github (http://github.com/HertzifyOS-Devices)." % device)
...
if not depsonly:
    githubreq = urllib.request.Request("https://api.github.com/search/repositories?q=%s+user:HertzifyOS-Devices+in:name+fork:true" % device)
    add_auth(githubreq)
    try:
...
def is_in_manifest(projectpath):
    ...
    # ... and don't forget the hertzify snippet
    try:
        lm = ElementTree.parse(".repo/manifests/snippets/hertzify.xml")
        lm = lm.getroot()
    except:
        lm = ElementTree.Element("manifest")
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        repo_remote = repository.get("remote", "hertzify-devices")
        repo_revision = repository.get('revision')

        if repo_remote != "hertzify-devices" and not repo_remote.startswith("aosp-"):
        ...
        if repo_revision and not (repo_remote == "hertzify-devices" and repo_revision == get_default_or_fallback_revision(repo_name)):
...
def fetch_dependencies(repo_path):
    ...
    dependencies_path = repo_path + '/hertzify.dependencies'
    syncable_repos = []
    verify_repos = []
...
def get_default_or_fallback_revision(repo_name):
    default_revision = get_remote_revision_from_snippet(".repo/manifests/snippets/hertzify.xml", "hertzify-devices")
...
print("Repository for %s not found in the HertzifyOS-Devices Github repository list. If this is an error, you may need to manually add it to your local_manifests/roomservice.xml." % device)